...
class AbsensiController extends Controller
{
    ...
    public function proses(Request $request)
    {
        ...
        // Mengambil nama role dari relasi dan mengubahnya ke huruf kecil
        ...
        if (!in_array($currentRoleName, $allowedRoles)) {
            return redirect()->route('absensi.scan')
                ->with('error', '❌ Akses ditolak! Role Anda (' . $currentRoleName . ') tidak diizinkan melakukan absensi.')
                ->withInput();
        }
        
        // Cari user yang akan di-absen berdasarkan barcode
        ...
        if (!$user) {
            return redirect()->route('absensi.scan')
                ->with('error', '❌ Barcode "' . htmlspecialchars($barcode) . '" tidak terdaftar.')
                ->withInput();
        }
        ...
        try {
            ...
            DB::table('absensi')->insert([
                ...
                'kategori' => $kategori,
                'lokasi' => 'Studio',
                'keterangan' => "Absen via scanner oleh: " . $currentUser->profil->nama_lengkap,
        ...
        try {
            ...
            if (!in_array($userRole, $allowedRoles)) {
                return response()->json([
                    'success' => false, 
                    'message' => 'Akses ditolak! Role ' . ($userRole ?: 'Member') . ' tidak bisa melakukan scan.'
                ], 403);
            }
    ...
    {
        $bulan = $request->get('bulan', date('Y-m'));
        $records = DB::table('absensi as a')
            ->leftJoin('profil_anggota as p', 'p.id_user', '=', 'a.id_user')
            ->select('p.nama_lengkap', 'a.tanggal', 'a.waktu', 'a.status', 'a.kategori')
            ->whereRaw("DATE_FORMAT(a.tanggal, '%Y-%m') = ?", [$bulan])
            ->get();
        
        $filename = "absensi_{$bulan}.csv";
        $handle = fopen('php://temp', 'w+');
        fputcsv($handle, ['Nama', 'Tanggal', 'Waktu', 'Status', 'Kategori']);
        foreach ($records as $r) fputcsv($handle, [$r->nama_lengkap, $r->tanggal, $r->waktu, $r->status, $r->kategori]);
        rewind($handle);
        $content = stream_get_contents($handle);
        fclose($handle);
        
        return response($content, 200)->header('Content-Type', 'text/csv')->header('Content-Disposition', "attachment; filename=\"$filename\"");
    }
